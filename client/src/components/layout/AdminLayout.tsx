...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-gray-950 border-r border-white/5 flex flex-col shrink-0 transition-transform duration-200 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold text-white">SMCar Admin</span>
          </div>
          {/* Mobile close button */}
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-lg font-medium transition-all duration-200 ${
        ...
        <div className="p-3 border-t border-white/5">
          <button
            ...
            className="flex items-center gap-3 w-full px-3.5 py-2.5 rounded-xl text-lg font-medium text-slate-600 hover:bg-white/5 hover:text-red-400 transition"
          >
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        {/* Mobile header */}
        <div className="lg:hidden flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white sticky top-0 z-30">
          ...
          <span className="text-lg font-bold text-gray-900">SMCar Admin</span>
        </div>
