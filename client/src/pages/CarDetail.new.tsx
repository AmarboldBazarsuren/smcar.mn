        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="lg:col-span-4">
            <div className="lg:sticky lg:top-24 space-y-4">
              ...
              <button
                onClick={() => {
                  const url = `${window.location.origin}/cars/detail/${car ? carDetailPublicId(car) : id}`
                  navigator.clipboard.writeText(url).then(() => {
                    setCopied(true)
                    setTimeout(() => setCopied(false), 2000)
                  })
                }}
                className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 text-white py-3 rounded-2xl text-[14px] font-semibold transition shadow-lg shadow-red-900/20"
              >
                {copied ? (
                  <>
