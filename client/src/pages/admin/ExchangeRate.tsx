...
export default function ExchangeRatePage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Ханш тохиргоо</h1>
        <p className="text-lg text-gray-500 mt-1">KRW, EUR, USD → MNT ханшийн тохиргоо</p>
      </div>

      <div className="bg-white rounded-2xl border border-gray-200 p-6 max-w-lg">
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="space-y-5">
          <div>
            <label className="block text-base font-semibold text-gray-600 uppercase tracking-wider mb-2">1 KRW = MNT</label>
            <input type="number" step="0.01" value={wonToMnt} onChange={(e) => setWonToMnt(Number(e.target.value))} className={inputClass} />
            <p className="text-base text-gray-500 mt-1.5">Жишээ: 2.8</p>
          </div>
          <div>
            <label className="block text-base font-semibold text-gray-600 uppercase tracking-wider mb-2">1 EUR = MNT</label>
            <input type="number" step="1" value={euroToMnt} onChange={(e) => setEuroToMnt(Number(e.target.value))} className={inputClass} />
            <p className="text-base text-gray-500 mt-1.5">Жишээ: 3800</p>
          </div>
          <div>
            <label className="block text-base font-semibold text-gray-600 uppercase tracking-wider mb-2">1 USD = MNT</label>
            <input type="number" step="1" value={usdToMnt} onChange={(e) => setUsdToMnt(Number(e.target.value))} className={inputClass} />
            <p className="text-base text-gray-500 mt-1.5">Гаалийн татвар тооцоолоход ашиглана. Жишээ: 3450</p>
          </div>

          {rates?.updatedAt && (
            <p className="text-base text-gray-500">Сүүлд: {new Date(rates.updatedAt).toLocaleString('mn-MN')}</p>
          )}

          <div className="flex items-center gap-3">
            <button type="submit" disabled={mutation.isPending} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-lg font-semibold transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50">
              {mutation.isPending ? 'Хадгалж байна...' : 'Хадгалах'}
            </button>
            {saved && <span className="text-lg text-green-500 font-medium">Амжилттай!</span>}
          </div>
        </form>
      </div>
    </div>
