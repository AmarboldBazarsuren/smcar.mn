...
export default function CarDetail() {
    ...
    return (
      <main className="bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[13px] text-gray-400 mb-4">Энэ машин устгагдсан эсвэл зарагдсан байж магадгүй</p>
          <Link to="/cars" className="inline-flex items-center gap-2 bg-dark text-white text-[13px] font-semibold px-5 py-2.5 rounded-lg hover:bg-gray-800 transition">
  ...
  const priceInfo = calcPrice(car.price, car.currency, rates, fees, car.original_price_krw, encarPrice, cc, car.year, car.fuelType)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="relative bg-black rounded-xl overflow-hidden">
              ...
              {/* Counter badge */}
              {imgs.length > 0 && (
                <div className="absolute bottom-3 right-3 bg-black/70 backdrop-blur-sm text-white text-[11px] font-medium px-3 py-1.5 rounded-full">
            ...
            <div className="lg:sticky lg:top-[76px] space-y-4">
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {priceInfo.mntCarPrice && (
                  <div className="px-5 py-4 bg-primary/5 border-b border-gray-100">
                    <p className="text-[11px] text-gray-500 uppercase tracking-wider font-medium mb-1">Машины үнэ</p>
                    <p className="text-[22px] font-extrabold text-primary">{priceInfo.krwFull}</p>
                    <p className="text-[14px] font-semibold text-gray-500 mt-0.5">{priceInfo.mntCarPrice}</p>
                  </div>
                )}

                {/* Fee breakdown */}
                <div className="p-5 space-y-3">
                  <p className="text-[11px] text-gray-400 uppercase tracking-wider font-semibold mb-2">Зардлын задаргаа</p>

                  {fees && rates && (
                    <>
                      <FeeRow label="Монгол үйлчилгээний шимтгэл" amountKrw={fees.serviceFee} wonToMnt={rates.wonToMnt} />
                      <FeeRow label="Тээврийн зардал" amountKrw={fees.transportFee} wonToMnt={rates.wonToMnt} />
                      ...
                      <div className="border-t border-gray-200 pt-3 mt-3">
                        <div className="flex justify-between">
                          <span className="text-[14px] font-bold text-dark">Нийт гаалийн зардал</span>
                          <span className="text-[14px] font-extrabold text-dark">{formatNumber(priceInfo.totalCustomsMnt)}₮</span>
                        </div>
                        <div className="flex justify-between mt-2 pt-2 border-t border-gray-100">
                          <span className="text-[15px] font-bold text-primary">Бүгд нийт</span>
                          <span className="text-[15px] font-extrabold text-primary">{priceInfo.mntGrandTotal}</span>
                        </div>
                      </div>
                    </>
                  )}
              ...
              {encarUrl && (
                <a
                  ...
                  className="flex items-center justify-center gap-2.5 w-full bg-white border border-gray-200 text-dark py-3.5 rounded-xl text-[13px] font-semibold hover:bg-gray-50 transition"
                >
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-blue-700 text-white py-4 rounded-xl text-[15px] font-bold transition-colors shadow-lg shadow-primary/20"
              >
                Захиалга өгөх
              </button>
              ...
              {/* Info card */}
              <div className="bg-blue-50 rounded-xl p-4 text-[12px] text-blue-700 leading-relaxed">
...
function calcPrice(
  ...
  const transportMnt = fees && rates ? Math.round(fees.transportFee * rates.wonToMnt) : 0
